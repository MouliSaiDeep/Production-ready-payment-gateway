...
export default function Checkout() {
  ...
  // --- UPDATED: CREATE TEST ORDER WITH HEADERS ---
  const createTestOrder = async () => {
    try {
      setLoading(true);
      // Create a random order for ₹500 - ₹5000
      ...
      const res = await axios.post(
        ...
        {
          // FIX: Add the required authentication headers
          headers: {
      ...
      // Auto-load the new order
      setManualOrderId(res.data.id);
      fetchOrder(res.data.id);
    } catch (err) {
      console.error(err);
      setError("Failed to create test order. Check console (F12) for details.");
      setLoading(false);
    }
  };
  // --------------------------------------
  ...
  // ... (Rest of the Payment UI remains exactly the same)
  return (
      ...
      {paymentState === "success" && (
        <div data-test-id="success-state" className="state-container">
          ...
          <p
            ...
            style={{ marginTop: "1.5rem", color: "var(--text-main)" }}
          >
